...
from config.endpoints import Admin_ORG_ENDPOINT, Admin_USER_ENDPOINT, CATEGORIES_ENDPOINT, PLACES_ENDPOINT, RISKS_ENDPOINT
...
class TestRisksAPI:
    ...
    @pytest.fixture
    def test_post_org(self, auth_api_admin):
        ...
        with allure.step("Создание организации"):
            ...
            (Assertions.assert_code_status(response2,200),
             f"Ожидался статус 200, но получен {response2.status_code}")
        ...
        with allure.step("Создание пользователя в организации"):
            ...
            (Assertions.assert_code_status(response3, 200),
             f"Ожидался статус 200, но получен {response3.status_code}")
        ...
        with allure.step("Присвоение пароля созданному пользователю"):
            ...
            (Assertions.assert_code_status(response4, 200),
             f"Ожидался статус 200, но получен {response4.status_code}")
        ...
        with allure.step("Присвоение роли администратор пользователю"):
            ...
            (Assertions.assert_code_status(response5, 200),
                 f"Ожидался статус 200, но получен {response5.status_code}")
    ...
    @pytest.fixture
    def test_post_directory(self, auth_api, test_post_org):
        ...
        with (allure.step("Создание новой категории")):
            ...
            (Assertions.assert_code_status(response2, 200),
             "Ожидался статус {expected_status}, но получен {response.status_code}n")
            Assertions.assert_json_has_key(response2, "id")
            cat_id = response2.json().get("id")
            print(f"Создана категория {cat_id}")

        with (allure.step("Создание новой территории")):
            response2 = requests.post(PLACES_ENDPOINT, headers=response1, json=place_data)
            (Assertions.assert_code_status(response2, 200),
             "Ожидался статус {expected_status}, но получен {response.status_code}n")
            Assertions.assert_json_has_key(response2, "id")
            place_id = response2.json().get("id")
    ...
    def test_create_risk(self, auth_api, test_post_directory):
