...
def test_post_org(auth_api_admin):
    ...
    with allure.step("Создание организации"):
        response2 = Myrequests.post(Admin_ORG_ENDPOINT, headers=response1, json=org_data)
    ...
    with allure.step("Создание пользователя в организации"):
        ...
        response3 = Myrequests.post(Admin_USER_ENDPOINT, headers=response1, json=user_data)
    ...
    with allure.step("Присвоение пароля созданному пользователю"):
        ...
        response4 = Myrequests.patch(Admin_USER_patch_ENDPOINT, headers=response1, json=password_data)
    ...
    with allure.step("Присвоение роли администратор пользователю"):
        response5 = Myrequests.post(Admin_role_ENDPOINT, headers=response1, json=role_admin_data)
    ...
    with allure.step("Присвоение роли риск-менеджер пользователю"):
        response6 = Myrequests.post(Admin_role_ENDPOINT, headers=response1, json=role_rm_data)
...
def test_post_directory(auth_api, test_post_org):
    ...
    with (allure.step("Создание новой категории")):
        response2 = Myrequests.post(CATEGORIES_ENDPOINT, headers=response1, json=category_data)
        Assertions.assert_code_status(response2, 200)
    ...
    with (allure.step("Создание новой территории")):
        response3 = Myrequests.post(PLACES_ENDPOINT, headers=response1, json=place_data)
        Assertions.assert_code_status(response3, 200)
...
def test_create_risk(auth_api, test_post_directory):
    ...
    with (allure.step("Создание нового риска")):
        ...
        response2 = Myrequests.post(RISKS_ENDPOINT, headers=response1, json=RISK_DATA)
        Assertions.assert_code_status(response2, 200)
        Assertions.assert_json_has_key(response2, "id")
        print(f"Создан риск {response2.json().get("id")}")

    with (allure.step("Получение списка рисков")):
        response3 = Myrequests.get(RISKS_ENDPOINT, headers=response1)
        Assertions.assert_code_status(response3, 200)
